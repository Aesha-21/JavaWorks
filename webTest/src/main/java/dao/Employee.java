...
//		}
//		
		int a = Integer.compare(this.level, e.level);
		if (a!=0)
				return a;
	
		
		int b = Integer.compare(this.exp, e.exp);
		if (b!=0)
			return b;
			
		int c = this.gender.compareTo(e.gender);
		if (c!=0)
			return c;
		
		int d = Float.compare(this.salary, e.salary);
		return d;
