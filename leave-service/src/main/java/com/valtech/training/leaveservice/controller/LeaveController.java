...
public class LeaveController {
	...
	public LeaveVO getLeave(@PathVariable("id") long id) {
		...
		LeaveVO lvo = leaveService.getLeave(id);
		EmployeeVO man = leaveService.getmanager(lvo.employeeId());
		System.out.println("Employe =" +lvo.employeeId()+"Manager"+man.id());
	...
	@PutMapping("/{id}")
	public LeaveVO createLeave(@PathVariable("id") long id,@RequestBody LeaveVO vo) {
		return leaveService.update(id,vo);
	}
